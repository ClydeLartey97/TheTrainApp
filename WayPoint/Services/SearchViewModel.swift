...
final class SearchViewModel {
    ...
    private(set) var lastCompletedRouteSelection: RouteStationSelection?
    ...
    func applyRoute(_ route: SavedRoute) {
        ...
        selectedOriginStation = route.originStation
        selectedDestinationStation = route.destinationStation
        originSuggestions = []
        destinationSuggestions = []
        isShowingOriginSuggestions = false
        isShowingDestinationSuggestions = false
    ...
    var currentRouteSelection: RouteStationSelection? {
    ...
    private func resolvedRouteSelection() -> RouteStationSelection? {
        let trimmedOrigin = origin.trimmingCharacters(in: .whitespacesAndNewlines)
        let trimmedDestination = destination.trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        let destinationStation = resolvedStation(
            for: trimmedDestination,
            selected: selectedDestinationStation,
            suggestions: destinationSuggestions
        )

        if !trimmedDestination.isEmpty && destinationStation == nil {
            return nil
        }

        return RouteStationSelection(origin: originStation, destination: destinationStation)
    }
